...
public class LinkedListLab<AnyType> implements Iterable<AnyType> {
    ...
    public AnyType kthToLast(int k) {
        ...
        try{ // TODO your implementation here
            int front = 0;
            int back = 0;
            int gap = 0;
            Iterator<AnyType> iteration = iterator();
            while(iteration.hasNext()) {
                front++ ;
                gap = front - back;
                if (gap > k) { // Check the edge case: k < 0
                    back++ ;
                }
                iteration.next();
            }
            if (k > front) { // Check the edge case: k > length
                return null;
            }
            for (Object x : this) {
                if (back  == 0) {
    ...
    private static class Node<AnyType> {
         ...
         * @param newNext next node reference
         */
    ...
    public static void main(String[] args) {
        ...
        // should print "way"
        System.out.println("4:" + theList.kthToLast(4));
        // should print "null"
        System.out.println("-1:" + theList.kthToLast(-1));
        // should print "null"
