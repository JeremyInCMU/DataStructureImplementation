...
public class SortedLinkedList implements MyListInterface {
    ...
    public SortedLinkedList(String[] unsortted) {
        initLLD (unsortted, null, unsortted.length);
        insertionSort(head, null);
    }

    /**
     * Helper function to get rid of the 
     ...
     * @param depth
     */
    private void initLLD(String[] array, Node cur, int depth) {
        if (depth == 1) { // Base case
            return;
        } else { // Recursive case
            if (depth == array.length) {
                cur = new Node(array[array.length - depth]);
                head = cur;
            }
            cur.next = new Node(array[array.length - depth + 1]);
            initLLD(array, cur.next, depth - 1);
        }
    }
     ...
     * @param prev
     */
    private void insertionSort(Node cur, Node prev) {
        if (cur == null) { // Base case
            return;
        } else { // Recursive case
            if (sort(null, head, prev, cur)) { // If sort
                 insertionSort(prev.next, prev);
            } else {
                 insertionSort(cur.next, cur);
            }
        }
    }
     ...
     * @param end
     */
    private boolean sort(Node startPrev, Node start, Node endPrev, Node end) {
        if (start == end) {
            return false;
            ...
            System.out.println(start.val);
            System.out.println(end.val);
            return sort(start, start.next, endPrev, end);
        }
    }
    ...
    public void add(String value) {
        ...
        Node prevNode = searchForPosition(null, head, addedNode);
        if (prevNode != null) {  // Implement insert process
