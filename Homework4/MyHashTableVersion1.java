...
public class MyHashTable implements MyHTInterface {
    ...
    private void initHashTable(int cap) {        
        ...
        hashTable = new DataItem[capacity];
        collisionNum = 0;
        size = 0;
    }
    ...
    public void insert(String value) {
        if (value == null) { // Special case: null
            insertNull(null);
            return;
        }
    ...
    /**
     * Inserts null into hash table.
     * @param value null.
     */
    private void insertNull(String value) {
        for (int i = 0; i < capacity; i++) {
            if (hashTable[i] == null) {
                hashTable[i] = new DataItem(null, 1);
                size += 1;
                break;
            } else if (hashTable[i].value == null) {
                hashTable[i].frequency += 1;
                break;
            } else if (i == 0 && !contains(value) &&
                       i == hashValue(hashTable[i].value)) {
                collisionNum++;
            }
        }
    ...
    public boolean contains(String key) {
        ...
        for (int i = 0; i < capacity; i++) {
            if (hashTable[i] != null &&
                hashTable[i].value == null) { // Special case null.
                 if (key == null) {
                     return true;
                 }
                 continue;
    ...
    private void rehash() {
        ...
        for (int i = 0; i < oldCapacity; i++) {
            if (oldHashTable[i] != null &&
                oldHashTable[i].value == null) {
                int tempFreq = oldHashTable[i].frequency;
                while(tempFreq > 0) {
                    insertNull(oldHashTable[i].value);
                    tempFreq--;  
                }
            }
            else if  (oldHashTable[i] != null && 
